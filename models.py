...
from feature_selector_general import FeatureSelector
...
def get_model(name, **kwargs):
    ...
    if name == 'nn':
        ...
        patch_size = kwargs.setdefault('patch_size', 5)
        center_pixel = True
        model = HamidaFeatureSelection(n_bands, n_classes, lam=kwargs['lam'],patch_size=patch_size,headstart_idx=kwargs['headstart_idx'])
        lr = kwargs.setdefault('learning_rate', 0.01)
...
class HamidaFeatureSelection(HamidaEtAl):
    def __init__(self, input_channels, n_classes,lam, patch_size=5, dilation=1,headstart_idx=None):
        super(HamidaFeatureSelection, self).__init__(input_channels, n_classes, patch_size, dilation)
        self.feature_selector = FeatureSelector(self.input_channels, sigma=0.5, device="cuda:0",headstart_idx=headstart_idx)
        self.input_channels = input_channels
    ...
    def forward(self, x):
        if self.feature_selector.const_masking is None and \
                self.test and self.feature_selector.mask is None:
    ...
    def get_gates(self, mode):
        if mode == 'raw':
            ...
            return np.minimum(1.0, np.maximum(0.0, self.feature_selector.mu.detach().cpu().numpy() + 0.5))
        else:
...
class LeeEtAlFeatureSelection(LeeEtAl):
    def __init__(self, input_channels, n_classes):
        ...
        self.input_channels = input_channels
        self.feature_selector = FeatureSelector(self.input_channels, sigma=0.25, device="cuda:0")
        self.softmax = nn.Softmax()
        self.loss = nn.CrossEntropyLoss()
        self.reg = self.feature_selector.regularizer
        self.lam = 1.25#0.75
