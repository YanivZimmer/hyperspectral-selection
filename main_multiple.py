...
parser.add_argument('--model', type=str, default=None,
                    ...
                    "mou (1D RNN)")
#parser.add_argument('--bands', nargs="+", type=int, help="bands to use (mask) index start from 1",
#                    default=None)
...
group_train.add_argument('--epoch_second', type=int, help="Training epochs after feature selection")

group_train.add_argument('--lam', type=int, help="lam for regularization in feature selection")
...
for n_selected_bands in bands_amount:
    for run in range(N_RUNS):
        ...
        if MODEL == 'SVM_grid':
            ...
            hyperparams['headstart_idx'] = None #n_bands_to_selection[str(n_selected_bands)]
    ...
    #            model.update_bands(BANDS)
    #        if hasattr(model,"feature_selector"):
    #            model.feature_selector.set_mask
            try:
                ...
                if EPOCH2 is not None:
                    ...
                    if hasattr(model, "test"):
                        model.test = True
                    train(model, optimizer, loss, train_loader, EPOCH2,
                          scheduler=hyperparams['scheduler'], device=hyperparams['device'],
                          supervision=hyperparams['supervision'], val_loader=val_loader,
                          display=viz)
        ...
        acc, kappa, f1score = show_results(run_results, viz, label_values=LABEL_VALUES)
